...
def parse_txt_index(txt_path=None, encoding="utf-8", toc_pattern=None,
                    already_utf8=False, text_str=None):
    """返回 (utf8_path: Path, index: list[{title,start,end}])。

    轻量索引：Rust 只回传「标题 + 指向 UTF-8 文件的字节偏移」，正文不回传。
    ...
    - **Rust 优先**：所有规则（目录 / 晋江 / 变长后行断言等）先交给 Rust 翻译层匹配，
      保证未来切换到批处理 / 并行时匹配逻辑统一；仅 Rust 进程级失败时回退 Python re 保底。
    """
    pat = _pattern_to_str(toc_pattern)
    if text_str is not None:
        # Rust 优先：text_str 落 UTF-8 临时文件供 Rust 读取
        fd, tmp = tempfile.mkstemp(suffix=".utf8.txt", prefix="txt_epub_")
        os.close(fd)
        ...
        try:
            ...
            # Rust 进程级失败时才回退 Python re（仅保底）
            rx = re.compile(pat)
            index = _index_text_re(text_str, rx)
    ...
    if already_utf8:
        ...
        source = tmp
    # Rust 优先：所有规则先走 Rust 翻译层（见上方 docstring）
    try:
        index = _index_with_rust(source, pat)
    except Exception:
        # Rust 进程级失败时回退 Python re 保底
        rx = re.compile(pat)
        index = _index_with_re(source, rx)
    return Path(source), index
...
def pack_chapters(temp_path, index, out_dir, toc_pattern=None, book_title=""):
    ...
    try:
        ...
        res = subprocess.run(
            [str(_RUST_BIN), "--pack", "--source", str(temp_path),
